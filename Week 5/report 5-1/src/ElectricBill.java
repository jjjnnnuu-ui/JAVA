...
public class ElectricBill {
    ...
    public static long calculateUsageCharge(int usageKw, boolean isSupported) {
        ...
        if (isSupported && remainingKw > 0) {
            remainingKw = Math.max(0, remainingKw - 100); // 100Kw 무료 사용 후 남은 사용량
        }

        // 100Kw까지 (혹은 지원 가구의 경우 무료 구간 초과분부터)
        if (remainingKw > 0) {
            int currentKwBlock = Math.min(remainingKw, 100); // 첫 100Kw 블록
        ...
        // 101Kw 초과분부터
        if (remainingKw > 0) {
            int prevUsage = isSupported ? 100 : 0; // 지원 가구는 100Kw부터 요금 계산 시작
            ...
            // 500Kw 초과
            if (remainingKw > 0) {
                int blockStart = Math.max(501, prevUsage + 1);
                int blockEnd = usageKw; // No upper limit beyond 500Kw; up to actual usage
